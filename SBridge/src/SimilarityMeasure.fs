...
module SimilarityMeasure =
    ...
    let compareVulPatchFunctions (vulBlocks: ControlBlock list) (patchBlocks: ControlBlock list) (binaryBlocks: ControlBlock list) (vulOnlyBlocks: ControlBlock list) (patchOnlyBlocks: ControlBlock list) (checkLength: bool) : float * float =
        ...
        | [], _, _ | _, [], _ | _, _, [] -> (0.0, 0.0)
        | _ ->
            if vulOnlyBlocks.IsEmpty && not patchOnlyBlocks.IsEmpty then
                let patchMatches =
                    patchOnlyBlocks
                    |> List.exists (fun patchBlock ->
                        binaryBlocks
                        |> List.exists (fun binBlock ->
                            let score = calculateBlockSimilarity patchBlock binBlock true
                            score >= 0.7))
                printfn "patchMatches %A" patchMatches
                if patchMatches then (0.0, 1.0) else (0.0, 0.0)
            elif patchOnlyBlocks.IsEmpty && not vulOnlyBlocks.IsEmpty then
                let vulMatches =
                    vulOnlyBlocks
                    |> List.exists (fun vulBlock ->
                        binaryBlocks
                        |> List.exists (fun binBlock ->
                            let score = calculateBlockSimilarity vulBlock binBlock true
                            score >= 0.5))
                if vulMatches then (1.0, 0.0) else (0.0, 0.0)
            else
                let findBestMatches blocks =
                    ...
                    |> List.mapi (fun idx block ->
                        ...
                        |> List.map (fun (binIdx, binBlock) ->
                            let score = calculateBlockSimilarity block binBlock true
                            (binIdx, score))
                        |> List.maxBy snd
                        |> function
                ...
                let vulCoverage = if vulOnlyBlocks.IsEmpty then 0.0 else float vulWinners.Length / float vulOnlyBlocks.Length
                let patchCoverage = if patchOnlyBlocks.IsEmpty then 0.0 else float patchWinners.Length / float patchOnlyBlocks.Length

                let vulAvgSim = if vulWinners.IsEmpty then 0.0 else vulWinners |> List.averageBy (fun (_, _, _, score) -> score)
                let patchAvgSim = if patchWinners.IsEmpty then 0.0 else patchWinners |> List.averageBy (fun (_, _, _, score) -> score)

                let vulFinal = vulCoverage * 0.5 + vulAvgSim * 0.5
                let patchFinal = patchCoverage * 0.5 + patchAvgSim * 0.5
